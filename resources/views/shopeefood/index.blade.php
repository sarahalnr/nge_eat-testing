...
@include('components.tambah-modal-shopee')

<!-- Modal Edit Transaksi -->
@include('components.edit-modal-shopee')
...
function openEditModal(transactionId) {
    fetch(`/shopeefood/${transactionId}/edit-json`)
        .then(response => response.json())
        .then(data => {
            // Panggil fungsi showEditModal dari edit-modal.blade.php
            showEditModal(data);
            // Set action form
            document.getElementById('formEditTransaksi').action = `/shopeefood/update/${transactionId}`;
        })
        .catch(err => {
    ...
    document.getElementById('formEditTransaksi').action = `/shopeefood/${transactionId}?page=${currentPage}`;
}
