...
<script>
  ...
  const shopeePlatformId = platformList.find(p => p.name.toLowerCase().includes('shopee'))?.id || '';
</script>
    ...
    <form id="formEditTransaksi" method="POST">
      ...
      <div class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm text-gray-700">
        ...
        <div>
          ...
          <input id="edit_id_pesanan" name="id_pesanan" type="text" class="border rounded-sm px-2 py-1 w-full bg-gray-100 shadow-sm" style="border-color: #F58220;" readonly>
        </div>
        <div>
...
<script>
  ...
  const shopeePlatformId = platformList.find(p => p.name.toLowerCase().includes('shopee'))?.id || '';
  const platformId = item.platform_id || shopeePlatformId;
