    ...
    <form id="formTambahTransaksi" action="{{ route('gofood.store') }}" method="POST">
      ...
      <div class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm text-gray-700">
        ...
        <div>
          ...
          <input id="id_pesanan" name="id_pesanan" type="text" class="border rounded-sm px-2 py-1 w-full bg-gray-100 shadow-sm" style="border-color: #F58220;" value="{{ $generatedId ?? '' }}" readonly>
        </div>
...
<script>
  ...
  function addTambahItemRow() {
    ...
    const gofoodPlatform = platformList.find(p => p.name.toLowerCase().includes('gofood'));
    const platformId = gofoodPlatform ? gofoodPlatform.id : '';
