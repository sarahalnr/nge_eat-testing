<div id="BerhasilTambahModal" class="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
  <div class="bg-white rounded-md shadow-md p-6 w-full max-w-xl flex flex-col items-center">
    ...
    <div class="mt-2 text-center">
      <h2 class="text-base mb-4">Transaksi berhasil ditambah</h2>
      <button id="closeBerhasilTambahModal" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 rounded">
        OK
      </button>
    </div>
...
<script>
    ...
    document.getElementById('closeBerhasilTambahModal').addEventListener('click', closeModal);

    // Modal Berhasil Download
    document.getElementById('openBerhasilTambahModal').addEventListener('click', function () {
        document.getElementById('BerhasilTambahModal').classList.remove('hidden');
    });
    ...
    document.getElementById('closeBerhasilTambahModal').addEventListener('click', closeAllModals);
</script>
