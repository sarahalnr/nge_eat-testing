                ...
                <tr class="border-t hover:bg-gray-50" data-tanggal="{{ $transaction->tanggal->format('Y-m-d') }}">
                    ...
                    <td class="px-6 py-3">
                        <div class="flex justify-center items-center space-x-4 text-gray-500">
                            ...
                            <form action="{{ route('grabfood.destroy', $transaction->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus transaksi ini?')" class="flex items-center justify-center m-0 p-0">
                                @csrf
                                @method('DELETE')
...
@include('components.tambah-modal-grab')

<!-- Modal Edit Transaksi -->
@include('components.edit-modal-grab')
...
function openEditModal(transactionId) {
    fetch(`/grabfood/${transactionId}/edit-json`)
        .then(response => response.json())
        .then(data => {
            // Panggil fungsi showEditModal dari edit-modal.blade.php
            showEditModal(data);
            // Set action form
            document.getElementById('formEditTransaksi').action = `/grabfood/update/${transactionId}`;
        })
        .catch(err => {
    ...
    document.getElementById('formEditTransaksi').action = `/grabfood/update/${transactionId}?page=${currentPage}`;
}
